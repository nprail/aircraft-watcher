...
describe('aircraftKey', () => {
  test('prefers hex', () => {
    expect(aircraftKey({ hex: 'abc123', r: 'N1', flight: 'RCH1' })).toBe('hex:abc123');
  });
  test('falls back to tail (r field)', () => {
    expect(aircraftKey({ r: 'N12345', flight: 'RCH1' })).toBe('tail:N12345');
  });
  ...
  test('tail is uppercased', () => {
    expect(aircraftKey({ r: 'n1234' })).toBe('tail:N1234');
  });
});
